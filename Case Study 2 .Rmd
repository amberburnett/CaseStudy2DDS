...
# keep the following variables in the whole training data
...
# keep the same variables in the whole testing data
...
#############################################################
# Split the whole training data into 2 subsets: one training data: train and one testing data: test
...
# Method 1
# column 16 is variable Turnover
...
# Get Predicted attrition using KNN regression for the validation dataset without actual attrition
ft$Turnover.Pred = results2

# Export data with predicted attrition
write.table(ft, "~/Desktop/Case2PredictionsBurnett knn Attrition.csv", sep=",", row.names=F)

#############################################################
# Run logistic model to compare
...
# Testing accuracy of the training data -subset of the whole training data (casestudy2data)
...
# Testing accuracy of the testing data -subset of the whole training data (casestudy2data)
...
# Success rate of our prediction = 1 -error rate

mean(train$turnover.pred==train$Turnover)


# Get Predicted attrition using Logistic regression for the validation dataset without actual attrition
...
#############################################################
# Use Linear Model to predict monthly income
...
# Get the accuracy of the model using testing data: as a subset of the training data (casestudy2 data)
ASE_lm <- mean((test$MonthlyIncome-MSalary.Pred)^2)
RMSE <- sqrt(ASE_lm)
RMSE

# Get the accuracy of the model using the whole training data (casestudy2 data)
MSalary.Predall <- predict(mod, case)
ASE_lm_all <- mean((case$MonthlyIncome-MSalary.Predall)^2)
RMSE_all <- sqrt(ASE_lm_all)
RMSE_all

# Get Predicted monthly salary using linear regression for the validation dataset without actual salary
...
# Export data with predicted salary
